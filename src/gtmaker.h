...
private:
    ...
    void select(const int id) {
        m_select = max(0, min(m_database.imNames.size() - 1, id));
    ...
    void adjustImg() {
        ...
        m_viewScale = 0.92 * min(static_cast<double>(m_wcam.dsize[0] - 180) / m_img.dsize[0], static_cast<double>(m_wcam.dsize[1]) / m_img.dsize[1]);
    }
